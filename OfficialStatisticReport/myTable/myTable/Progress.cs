...
namespace myTable
{
    ...
    {
        // 標記此校是否已執行過預設類別初始化，避免每次應用程式啟動(模組載入)都重複查詢/寫入標籤資料
        private const string 預設類別初始化標記Key = "新生入學統計報表_預設類別已初始化";
        ...
        // 確保此校已建立預設的入學方式、入學身分、原住民類別標籤；只需執行一次，之後即略過。
        // 刻意不放在 main() (模組載入時機)呼叫，避免每次應用程式啟動都需檢查/建立，拖慢模組載入速度；
        // 改由 Form2 開啟報表時於背景執行緒呼叫，只有真正用到這份報表時才需要付出這個(至多一次)的成本。
        public static void EnsureDefaultTagsBootstrapped()
        {
            ConfigData 初始化標記 = K12.Data.School.Configuration[預設類別初始化標記Key];

            if (初始化標記.GetXml("XmlData", null) == null)
            {
                ...
                XmlElement doneMarker = new XmlDocument().CreateElement(預設類別初始化標記Key);
                doneMarker.SetAttribute("done", "1");
                初始化標記.SetXml("XmlData", doneMarker);
                初始化標記.Save();
            }
        }
    }
